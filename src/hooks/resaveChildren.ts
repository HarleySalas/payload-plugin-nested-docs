import { CollectionAfterChangeHook, CollectionConfig, PayloadRequest } from "payload/types"
...
const resave = async ({ collection, doc, draft, pluginConfig, req }: ResaveArgs) => {
	...
	try {
		await children.docs.reduce(async (priorSave, child) => {
			...
			const childIsPublished = typeof collection.versions === 'object' &&
		...
		req.payload.logger.error(
			`Nested Docs plugin has had an error while re-saving a child document${ draft ? ' as draft' : ' as published'}.`
		)
